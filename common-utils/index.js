...
const generateRandomString = (length = 15, char_type = 1) => {
    ...
    let characters_str = characters[char_type],
        charactersLength = characters_str.length,
        randomString = '',
        i = 0;
    for (; i < length; i++) {
...
function replace_str_arr(replaceArray, replaceArrayValue, string) {
    let finalAns = string;
    let i = replaceArray.length - 1;
    for (; i >= 0; i--) {
        finalAns = finalAns.replace(
            RegExp('\\b' + replaceArray[i].replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + '\\b', 'g'),
            replaceArrayValue[i],
        );
    }
    return finalAns;
...
function error_log_message(
    ...
    array['type'] = type;
    array['log'] = JSON.stringify(log_string).replace(
        /'/gi,
        '',
    ); /* remove single ' for insert db in error log*/
    return array;
}
        ...
        if (Array.isArray(r)) {
            if (Array.isArray(v)) {
                ...
                for (let i = r.length - 1; i >= 0; i--) {
                    temp = temp.replace(
                        new RegExp(r[i], 'g'),
                        v[i] ? v[i] : v[v.length - 1] ? v[v.length - 1] : '',
                    );
                }
                return temp;
            } else {
...
const date_array_bet = (start_date, end_date, date_diff, hours, arr_label) => {
    ...
    while (key--) {
        ...
        date_arr[Date.parse(date_format(dateformat, newdate))] = {
            [arr_label[0]]: 0,
            [arr_label[1]]: 0,
        };
    }
...
const funnel_valid_check = (funnel_arr) => {
    ...
    while (funnel_arr_len--) {
        ...
        if (
            condition_val_type != json_check.ALL_ORDERS &&
            json_check.NOT_CHECK_CONDITION.indexOf(funnel_arr[funnel_arr_len].condition) == -1
        ) {
            if (
                funnel_arr[funnel_arr_len].condition_values &&
                funnel_arr[funnel_arr_len].back_conditions_values
            ) {
                if (
                    json_check.CHECK_BETWEEN_NOT_CONDITION.indexOf(funnel_arr[funnel_arr_len].condition) != -1
                ) {
                    const condition_val_arr =
                        funnel_arr[funnel_arr_len].condition_values.indexOf('@$@') != -1
                            ? funnel_arr[funnel_arr_len].condition_values.split('@$@')
                            : [];
                    if (
                        condition_val_arr.length > 0 &&
                        !isNaN(condition_val_arr[0]) &&
                        !isNaN(condition_val_arr[1])
                    ) {
                        continue;
                    } else {
                        err_msg += `<li> Please provide valid values for ${
                            funnels_obj[funnel_arr[funnel_arr_len].category][funnel_arr[funnel_arr_len].parameter]
                        };</li>`;
                    }
                } else if (
                    json_check.CHECK_NUMBERS.indexOf(condition_val_type) != -1 &&
                    isNaN(funnel_arr[funnel_arr_len].condition_values)
                ) {
                    err_msg += `<li> Please provide valid values for ${
                        funnels_obj[funnel_arr[funnel_arr_len].category][funnel_arr[funnel_arr_len].parameter]
                    };</li>`;
                ...
                err_msg += `<li>Please provide necessary values for ${
                    funnels_obj[funnel_arr[funnel_arr_len].category][funnel_arr[funnel_arr_len].parameter]
                };</li>`;
            }
        }
...
const customEval = (str) => {
    try {
        ...
        return new Function(
            `'use strict'; const TRUE = true; const FALSE = false; const AND='&&'; return ${str};`,
        )();
    } catch (error) {
        return false;
    }
